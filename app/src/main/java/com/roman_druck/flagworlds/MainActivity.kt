...
import com.roman_druck.flagworlds.data.FWRepository
import com.roman_druck.flagworlds.data.FWdatabase
import com.roman_druck.flagworlds.data.FillDataBase
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        chkBoxes.add(findViewById(R.id.ck7))
        sharedPref = getSharedPreferences("fwprefs", Context.MODE_PRIVATE)
    ...
    fun checkInput(playGame: Boolean = true) {
        ...
        if (checkBoxesEmpty()) {
            ...
            if (ck2.isChecked) {
                ck1.isChecked = true //If Europe is choden then chose also East Europe
            }
            DataManager.flagLista.clear() //Clear the DataManager
            //Load datamanager in a Coroutine
            CoroutineScope(IO).launch {
                if (ck7.isChecked){
                    loadDataManager(context, "unrecognized_states")
                }
                if (ck6.isChecked){
                ...
                if (ck1.isChecked){
                    loadDataManager(context,"easteurope")
                }
    ...
    suspend fun loadDataManager(context: Context, continent: String){
        val repository = FWRepository(FWdatabase.getDatabase(context).FWdao())
        CoroutineScope(IO).launch {
            val list = repository.getContinent(continent)
            list.forEach {
                DataManager.flagLista.add(Flag(it.enCountry, it.uaCountry, it.ruCountry, it.flagId))
            }
        }
    }
